...
.PARAMETER MatchesOnlyPhones
  If set, exports ONLY Matches_*.csv and only for -Phone / -PhonePartial
  (skips email matching, duplicates, shadow candidates, and inventory).
...
.PARAMETER ExportInventory
  Also export per-user inventory CSV (ignored when -MatchesOnlyPhones is set).
...
# search sets (email ignored when MatchesOnlyPhones)
$searchEmails = @()
if (-not $MatchesOnlyPhones){
  foreach($e in @($Email)){ $n=Normalize-Email $e; if($n){ $searchEmails += $n } }
}
...
# control toggles
$collectMaps     = -not $MatchesOnlyPhones
$buildInventory  = $ExportInventory -and -not $MatchesOnlyPhones
$buildCandidates = -not $MatchesOnlyPhones
...
foreach($u in $users){
  ...
  foreach($v in @($u.UserPrincipalName,$u.Mail)){
    $n=Normalize-Email $v; if($n){
      $uEmails += $n; if($collectMaps){ Add-ToMap -Map $emailMap -Key $n -Value $u.Id }
    }
  }
  ...
  foreach($v in @($u.ProxyAddresses)){
    ...
    if ($m.Success) {
      ...
      if($n){ $uEmails += $n; if($collectMaps){ Add-ToMap -Map $emailMap -Key $n -Value $u.Id } }
    }
  }

  foreach($v in @($u.MobilePhone)){
    $n=Normalize-Phone $v; if($n){
      $uPhones += $n; if($collectMaps){ Add-ToMap -Map $phoneMap -Key $n -Value $u.Id }
    }
  }
  foreach($v in @($u.BusinessPhones)){
    foreach($z in @($v)){
      $n=Normalize-Phone $z; if($n){
        $uPhones += $n; if($collectMaps){ Add-ToMap -Map $phoneMap -Key $n -Value $u.Id }
      }
    }
  }
  ...
  foreach($idn in @($u.Identities)){
    ...
    switch ($idn.SignInType) {
      'emailAddress'       { $n=Normalize-Email $iss; if($n){ $uEmails += $n; if($collectMaps){ Add-ToMap -Map $emailMap -Key $n -Value $u.Id } } }
      'userPrincipalName'  { $n=Normalize-Email $iss; if($n){ $uEmails += $n; if($collectMaps){ Add-ToMap -Map $emailMap -Key $n -Value $u.Id } } }
      'phoneNumber'        { $n=Normalize-Phone $iss; if($n){ $uPhones += $n; if($collectMaps){ Add-ToMap -Map $phoneMap -Key $n -Value $u.Id } } }
      default { }
    }
  }
  ...
  foreach($v in $auth.Emails){
    $n=Normalize-Email $v; if($n){ $uEmails += $n; if($collectMaps){ Add-ToMap -Map $emailMap -Key $n -Value $u.Id } }
  }
  foreach($v in $auth.Phones){
    $n=Normalize-Phone $v; if($n){ $uPhones += $n; if($collectMaps){ Add-ToMap -Map $phoneMap -Key $n -Value $u.Id } }
  }
...
# duplicates & shadow pairs (only when not matches-only)
$dupeRows = @()
if ($buildCandidates){
...
# Matches-only (phones) fast path
if ($MatchesOnlyPhones){
  ...
  Write-Host ("Matches (phones) : {0}" -f $pathMat)
  return
}
...
if ($buildInventory){ $invRows | Sort-Object DisplayName | Export-Csv -NoTypeInformation -Encoding UTF8 -Path $pathInv }
$matches         | Sort-Object Type,Value,DisplayName | Export-Csv -NoTypeInformation -Encoding UTF8 -Path $pathMat
...
if ($buildInventory){ Write-Host ("Inventory         : {0}" -f $pathInv) }
