...
function ConvertTo-IsoDuration {
  <# Accepts: 90, 90m, 0.5h, 0,5h, 1h, 1h30m, 2h30m, 1d, 01:30:00, PT1H30M #>
  ...
  if ($s -match '(\d+)\s*m(?!in)') {
    # only match plain 'm' for minutes (not part of something else)
  ...
  if ($d -eq 0 -and $h -eq 0 -and $m -eq 0) {
    throw "Invalid duration '$Value'. Examples: 30m, 0.5h, 1h, 1h30m, 2h, 1d, 01:30:00, PT45M"
  }
  if ($d -gt 0) { return "P${d}D" }
  # normalize overflow: 90m -> 1h30m
  ...
  switch ($Action) {
    ...
    'Activate' {
      ...
      $rawDur = if ($Duration) {
        ...
        $ans = Read-Host "Enter duration (e.g. 30m, 1h, 1h30m, 2h) [default 1h]"
        if ([string]::IsNullOrWhiteSpace($ans)) { '1h' } else { $ans }
      }
